...
type Cache interface {
	Get(key string) (*MISPThreatIntelligence, bool)
	Set(key string, intel *MISPThreatIntelligence, ttl time.Duration)
	Delete(key string)
	Clear()
	Close() error
}
...
func (mc *MemoryCache) Get(key string) (*MISPThreatIntelligence, bool) {
	mc.mu.RLock()
	defer mc.mu.RUnlock()
...
func (mc *MemoryCache) Set(key string, intel *MISPThreatIntelligence, ttl time.Duration) {
	mc.mu.Lock()
	defer mc.mu.Unlock()
...
func (rc *RedisCache) Get(key string) (*MISPThreatIntelligence, bool) {
	...
	var intel MISPThreatIntelligence
...
func (rc *RedisCache) Set(key string, intel *MISPThreatIntelligence, ttl time.Duration) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
...
func NewCacheManager(useRedis bool, redisURL string, cacheSize int, logger *log.Logger) (*CacheManager, error) {
	...
	if useRedis && redisURL != "" {
		redisCache, err := NewRedisCache(redisURL, "misp:cache:", logger)
		if err != nil {
...
func (cm *CacheManager) Get(key string) (*MISPThreatIntelligence, bool) {
...
func (cm *CacheManager) Set(key string, intel *MISPThreatIntelligence, ttl time.Duration) {

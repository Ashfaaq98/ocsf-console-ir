	...
	handler := func(event *tcell.EventKey) *tcell.EventKey {
		...
		case tcell.KeyRune:
			...
			case ',':
				...
				return nil
			case 'L':
				// Shift+L opens the settings panel on the copilot, which is
				// where the provider form now lives.
...
func (cm *CaseManagement) sendCopilotMessage() {
	...
	if cm.parentUI != nil && tokensEst > cm.parentUI.prefs.copilotTokenWarning() {
...
// showLLMSettingsModal opens the copilot's provider settings.
//
// The same form the settings panel opens, so there is one place the provider,
// endpoint, model and key are edited however you got there. Shift+L inside a
// case is the shortcut to it.
func (cm *CaseManagement) showLLMSettingsModal() {
	if cm.parentUI != nil {
		cm.parentUI.showSettingsAt("Copilot")
		return
	}
	showLLMSettingsForm(llmSettingsHost{
		...
		mount:   cm.pushModalRoot,
		dismiss: cm.popModalRoot,
		status:  cm.updateStatus,
	}, func() {})
}

// getCurrentAnalyst returns case owner if present, else env var CONSOLE_IR_ANALYST, else "analyst".
// getCurrentAnalyst is who this screen attributes an action to.
//
// The same answer the rest of the application gives. It used to have its own
// rule — the case's owner first, then CONSOLE_IR_ANALYST, then a literal
// "analyst" — while everywhere else read USER. So one person's note could be
// signed with one name and their audit entry with another, and working someone
// else's case attributed your actions to *them*: an audit trail that says
// a.novak closed this case when p.osei closed it is not a record, it is a
// falsified one.
func (cm *CaseManagement) getCurrentAnalyst() string {
	if cm.parentUI != nil {
		return cm.parentUI.currentAnalyst()
	}
...
func (cm *CaseManagement) toggleCaseCopilot() {
	...
	// Switched off in settings means switched off: no drawer, no suggestions,
	// no request. Said rather than ignored, so the key does not read as broken.
	if cm.parentUI != nil && cm.parentUI.prefs.CopilotOff && !cm.copilotOpen {
		cm.updateStatus("The copilot is switched off — , opens settings")
		return
	}
	if cm.copilotOpen {
		if cm.copilotFull {

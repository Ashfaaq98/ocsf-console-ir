...
func newGroqTestServer(t *testing.T) (*httptest.Server, *int, *int) {
	t.Helper()
	chatCalls := 0
	modelsCalls := 0
...
func TestGroqChatSuccess(t *testing.T) {
	srv, chatCalls, _ := newGroqTestServer(t)
	defer srv.Close()

	provider, err := NewGroq(srv.URL, "llama-3.3-70b-versatile", "testkey", nil)
	if err != nil {
	...
	if resp.Message.Content == "" || !strings.Contains(resp.Message.Content, "Groq mock") {
...
func TestGroqChatNoModel(t *testing.T) {
	// Provider with empty model
	provider, err := NewGroq("https://api.groq.com/openai/v1", "", "key", nil)
	if err != nil {
		t.Fatalf("NewGroq error: %v", err)
	}
...
func TestGroqListModelsAndHealthCheck(t *testing.T) {
	srv, _, modelsCalls := newGroqTestServer(t)
	defer srv.Close()
...
func TestBuildGroqWithEnvAPIKeyAndDiscovery(t *testing.T) {
	srv, _, _ := newGroqTestServer(t)
	defer srv.Close()

	// Ensure env var is used when APIKey is blank
	_ = os.Setenv("GROQ_API_KEY", "env-key")
...
func TestGroqSummarizeCase(t *testing.T) {
	srv, _, _ := newGroqTestServer(t)
	defer srv.Close()

	p, err := NewGroq(srv.URL, "llama-3.3-70b-versatile", "testkey", nil)
	if err != nil {
	...
	if !strings.Contains(sum, "Groq mock") {

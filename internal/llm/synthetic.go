...
// Synthetic implements an LLM provider backed by Synthetic API (OpenAI-compatible API).
...
func NewSynthetic(endpoint, model, apiKey string, logger *log.Logger) (*Synthetic, error) {
	...
	if key == "" {
		key = strings.TrimSpace(os.Getenv("SYNTHETIC_API_KEY"))
	}
	if key == "" {
		return nil, fmt.Errorf("synthetic: apiKey required (set in settings or SYNTHETIC_API_KEY)")
	}
	return &Synthetic{
...
func (s *Synthetic) EstimateTokens(text string) int {
	return EstimateTokens(text)
}

// Chat implements ChatProvider using Synthetic's /chat/completions (OpenAI-style).
func (s *Synthetic) Chat(ctx context.Context, req ChatRequest) (*ChatResponse, error) {
	if strings.TrimSpace(s.model) == "" {
		return &ChatResponse{Error: "synthetic: model not configured"}, nil
	}
	...
	type synResp struct {
		...
		Model   string      `json:"model"`
		Choices []synChoice `json:"choices"`
		Usage   synUsage    `json:"usage"`
		Error   *struct {
	...
	msgs := make([]synMsg, 0, len(req.Messages))
	for _, m := range req.Messages {
		role := strings.ToLower(strings.TrimSpace(m.Role))
		if role == "" {
			role = "user"
		}
		msgs = append(msgs, synMsg{Role: role, Content: m.Content})
	}
	...
	if err != nil {
		return nil, fmt.Errorf("synthetic: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("synthetic: request error: %w", err)
	}
	defer resp.Body.Close()

	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("synthetic: status %d: %s", resp.StatusCode, truncateSyntheticBody(string(body), 400))
	}

	var parsed synResp
	if err := json.Unmarshal(body, &parsed); err != nil {
		return nil, fmt.Errorf("synthetic: decode response: %w", err)
	}
	if parsed.Error != nil {
		return &ChatResponse{Error: parsed.Error.Message}, nil
	}
	if len(parsed.Choices) == 0 {
		return &ChatResponse{Error: "synthetic: empty choices"}, nil
	}
	...
	// Cost: Synthetic pricing (using similar estimation to other providers)
...
func (s *Synthetic) SummarizeCase(ctx context.Context, case_ store.Case, events []store.Event) (string, error) {
	var sb strings.Builder
	...
	for i := 0; i < limit; i++ {
		...
		sb.WriteString(fmt.Sprintf("- %s | %s | sev=%s | host=%s | %s\n",
			...
			ev.Host,
			truncateSyntheticBody(ev.Message, 160),
		))
	}
	...
	if r == nil || r.Error != "" {
		if r != nil {
			return "", fmt.Errorf("synthetic: %s", r.Error)
		}
		return "", fmt.Errorf("synthetic: empty response")
	}
...
func (s *Synthetic) AnalyzeEvents(ctx context.Context, events []store.Event) (*EventAnalysis, error) {
	ls := &LocalStub{}
	return ls.AnalyzeEvents(ctx, events)
}

func (s *Synthetic) GenerateRecommendations(ctx context.Context, case_ store.Case, events []store.Event) ([]string, error) {
	ls := &LocalStub{}
...
// ListModels queries Synthetic /models and returns model IDs.
func (s *Synthetic) ListModels(ctx context.Context) ([]string, error) {
	type mdl struct {
		ID string `json:"id"`
	...
	url := s.endpoint + "/models"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, fmt.Errorf("synthetic list models: %w", err)
	}
	...
	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("synthetic list models: status %d: %s", resp.StatusCode, truncateSyntheticBody(string(body), 400))
	}
	var parsed mdlResp
	if err := json.Unmarshal(body, &parsed); err != nil {
		return nil, fmt.Errorf("synthetic list models: decode: %w", err)
	}
...
func (s *Synthetic) HealthCheck(ctx context.Context) error {
	url := s.endpoint + "/models"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return err
	...
	if resp.StatusCode/100 != 2 {
		body, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("synthetic health: status %d: %s", resp.StatusCode, truncateSyntheticBody(string(body), 300))
	}
	return nil
}
...
func truncateSyntheticBody(s string, max int) string {
	if max <= 0 {
		return ""
	}
	if len(s) <= max {

...
func (ls *LocalStub) Chat(ctx context.Context, req ChatRequest) (*ChatResponse, error) {
	...
	response := ls.generateChatResponse(userMessage, req.Persona)
...
func (ls *LocalStub) generateChatResponse(message, persona string) string {
	...
	case PersonaSOC, "IR Analyst":
		return ls.generateSOCAnalystResponse(msg)
	// Threat Hunter (canonical constant covers the literal)
	case PersonaHunter:
		return ls.generateThreatHunterResponse(msg)
	// Forensics: accept canonical and legacy "Forensics Expert"
	case PersonaForensics, "Forensics Expert":
		return ls.generateForensicsResponse(msg)
	default:
		return ls.generateGeneralResponse(msg)
	}
}

func (ls *LocalStub) generateThreatHunterResponse(message string) string {
...
func (ls *LocalStub) generateSOCAnalystResponse(message string) string {
...
func (ls *LocalStub) generateForensicsResponse(message string) string {
...
func (ls *LocalStub) generateGeneralResponse(message string) string {

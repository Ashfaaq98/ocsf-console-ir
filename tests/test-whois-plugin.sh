...
PLUGIN_BIN="$CONSOLE_DIR/bin/whois"
PLUGIN_LOG="$CONSOLE_DIR/logs/whois-plugin.log"
CONSUMER_NAME="whois-test"
...
# 2) Build the whois plugin inside golang:1.25, using the plugin's own go.mod to resolve dependencies
echo "Building whois plugin in golang:1.25 container (running as current user, plugin module)..."
$DOCKER_CMD run --rm \
  -u "$(id -u):$(id -g)" \
  -e HOME=/tmp \
  ...
  -w /workspace/console-ir/plugins/whois \
  golang:1.25 sh -c 'mkdir -p /tmp/.cache/go-build /workspace/go/pkg/mod && go env -w GOMODCACHE=/workspace/go/pkg/mod && go mod tidy && go build -o /workspace/console-ir/bin/whois .'
...
# 4) Start whois plugin (background) — plugin connects to Redis on localhost:6379
echo "Starting whois plugin (background)..."
mkdir -p "$CONSOLE_DIR/logs"
cd "$CONSOLE_DIR/tests"
"$PLUGIN_BIN" --redis "redis://localhost:6379" --consumer "$CONSUMER_NAME" > "$PLUGIN_LOG" 2>&1 &
PLUGIN_PID=$!
echo "Whois plugin started with PID $PLUGIN_PID; logs -> $PLUGIN_LOG"
cd "$REPO_ROOT"

# give plugin & redis a moment
sleep 2

# 5) Publish a test event containing a URL/domain
JSON='{"url":"http://example.com","description":"test whois event"}'
TS=$(date +%s)
echo "Publishing test event to Redis (events stream)..."
$DOCKER_CMD exec -i "$REDIS_CONTAINER" redis-cli XADD events "*" event_id evt_whois_1 event_type network raw_json "$JSON" timestamp "$TS"
...
echo ""
echo "If you see enrichment messages with source=whois and data containing keys like 'whois_example_com_registrar', processing worked."
